...
> **おすすめの投稿時間帯**: 朝8時・昼12時・夜20〜22時が国内のエンゲージメントが高い時間帯です。分析結果 (`/analytics_report`) で最適な時間帯を確認し、スロットを調整できます。分析は6時間ごとに自動実行されるため、数日運用するだけで参考データが蓄積されます。
...
### Step 5-1: メトリクスは自動で蓄積される

ボットを起動すると、`ANALYTICS_FETCH_INTERVAL_HOURS`（デフォルト: **6時間**）ごとに全ゲームの直近90日のツイートメトリクスが **自動取得・蓄積** されます。ユーザーが手動でコマンドを呼ぶ必要はありません。

取得されたデータは2か所に記録されます：

| 保存先 | 内容 |
|---|---|
| `tweets` テーブル | 最新のインプレッション・いいね・RT・リプライ数（常に最新値に更新） |
| `tweet_metrics_history` テーブル | 取得ごとのスナップショット（時系列で蓄積、削除されない） |

> **設定**: `.env` の `ANALYTICS_FETCH_INTERVAL_HOURS` を変更すると取得間隔を調整できます（例: `3` にすると3時間ごと）。

---

### Step 5-2: 今すぐ手動でメトリクスを取得する (`/analytics_fetch`)

自動取得を待たずにすぐ最新データを反映したい場合は、手動で実行できます。

```
/analytics_fetch game_id:niwa-kobito
```

> **動作**: 過去90日以内のツイートをすべて対象に、Twitter APIから最新値を取得して更新します（取得済み・未取得を問わず全件再取得）。実行後すぐに `/analytics_report` で分析できます。

---

### Step 5-3: インプレッションの時系列変化を確認する (`/analytics_history`)

特定のツイートがどのように伸びているか、時系列で確認できます。

```
/analytics_history tweet_id:1234567890123456789
```

**表示例**:
```
インプレッション履歴 / 1234567890123456789
取得回数: 4 回
────────────────────────
2026-04-01 08:00   👁 1,200  ❤️ 32  🔁 8  💬 3
2026-04-01 14:00   👁 2,800  ❤️ 67  🔁 19  💬 7
2026-04-02 02:00   👁 4,100  ❤️ 89  🔁 25  💬 9
2026-04-02 08:00   👁 4,350  ❤️ 91  🔁 26  💬 9
```

> **活用方法**: 投稿直後に伸びたのか、口コミで後から広がったのかがわかります。最大10件のスナップショットを表示します（それ以上ある場合は最新10件）。

---

### Step 5-4: 分析レポートを生成する (`/analytics_report`)
...
### Step 5-5: エンゲージメント上位投稿を確認する (`/analytics_top`)
...
⑥ 分析・改善（自動 + 月1回程度の確認）
   ↓ 自動（6時間ごと）
   メトリクスが自動取得され tweet_metrics_history に蓄積される
   ↓ 手動確認
   /analytics_history → 特定ツイートの時系列インプレッションを確認
   /analytics_fetch  → 今すぐ強制取得（自動取得を待たない場合）
